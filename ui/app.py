...
try:
    from core.scanners import (
        tcp_connect_scan, http_probe, get_ssl_info,
        FirewallEvasionConfig, FirewallEvader,
        tcp_connect_scan_with_evasion,
        IDSEvader, PassiveReconConfig,
        SecureLogger, ScanActivity
    )
except Exception as e:
    tcp_connect_scan = None
    http_probe = None
    get_ssl_info = None
    FirewallEvasionConfig = None
    FirewallEvader = None
    tcp_connect_scan_with_evasion = None
    IDSEvader = None
    PassiveReconConfig = None
    SecureLogger = None
    ScanActivity = None
...
class TcpConnectWorker(QThread):
    ...
    def run(self):
        if tcp_connect_scan is None:
            self.result.emit([])
        else:
            open_ports = tcp_connect_scan(self.host, self.ports, workers=50, timeout=0.5)
            self.result.emit(open_ports)
        self.finished.emit()

class HttpProbeWorker(QThread):
    ...
    def run(self):
        if http_probe is None:
            ...
            info = http_probe(self.host, self.port, timeout=2.0)
            self.result.emit(info)
        self.finished.emit()

class SslInfoWorker(QThread):
    result = pyqtSignal(dict)
    finished = pyqtSignal()
    ...
    def run(self):
        if get_ssl_info is None:
            self.result.emit({})
        else:
            info = get_ssl_info(self.host, self.port, timeout=3.0)
            self.result.emit(info)
        self.finished.emit()

class TcpConnectEvasionWorker(QThread):
    result = pyqtSignal(dict)
    finished = pyqtSignal()

    def __init__(self, host: str, ports: list, evasion_config):
        super().__init__()
        self.host = host
        self.ports = ports
    ...
    def run(self):
        try:
            from core.scanners import tcp_connect_scan_with_evasion
            scan_result = tcp_connect_scan_with_evasion(
                self.host, self.ports, evasion_config=self.evasion_config
            )
...
class MainWindow(QMainWindow):
    ...
    def _build_scan_options(self):
        ...
        # EVASION OPTIONS
        evasion_label = QLabel("EVASION OPTIONS")
        evasion_label.setStyleSheet("color: #ff9900; font-size: 11px; letter-spacing: 2px;")
        layout.addWidget(evasion_label)
        layout.addSpacing(4)

        self.evasion_enabled = QCheckBox("Enable Firewall Evasion")
        self.evasion_enabled.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_enabled)

        self.evasion_timing = QComboBox()
        self.evasion_timing.addItem("Timing: Normal", "normal")
        self.evasion_timing.addItem("Timing: Sneaky (slow)", "sneaky")
        self.evasion_timing.addItem("Timing: Polite", "polite")
        self.evasion_timing.addItem("Timing: Aggressive", "aggressive")
        self.evasion_timing.addItem("Timing: Insane", "insane")
        self.evasion_timing.setStyleSheet("""
            QComboBox {
                background-color: #111111;
                color: #ff9900;
        ...
        """)
        layout.addWidget(self.evasion_timing)
        layout.addSpacing(6)

        self.evasion_decoys = QCheckBox("Use Decoys")
        self.evasion_decoys.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_decoys)

        self.evasion_fragment = QCheckBox("Fragment Packets")
        self.evasion_fragment.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_fragment)

        self.evasion_randomize_ports = QCheckBox("Randomize Source Ports")
        self.evasion_randomize_ports.setChecked(True)
        self.evasion_randomize_ports.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_randomize_ports)

        self.evasion_randomize_ttl = QCheckBox("Randomize TTL")
        self.evasion_randomize_ttl.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_randomize_ttl)

        self.evasion_packet_size = QCheckBox("Vary Packet Size")
        self.evasion_packet_size.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        layout.addWidget(self.evasion_packet_size)
    ...
    def _build_scan_output(self):
        ...
        # quick lightweight scanners (TCP connect / HTTP probe / SSL info)
        btn_row = QWidget()
        btn_layout = QHBoxLayout(btn_row)
        btn_layout.setContentsMargins(8,8,8,8)
        self.btn_tcp_connect = QPushButton("TCP Connect")
        self.btn_http_probe = QPushButton("HTTP Probe")
        self.btn_ssl_info = QPushButton("SSL Info")
        for b in (self.btn_tcp_connect, self.btn_http_probe, self.btn_ssl_info):
            b.setFocusPolicy(Qt.FocusPolicy.NoFocus)
            b.setFixedHeight(28)
            btn_layout.addWidget(b)
        layout.addWidget(btn_row)

        self.btn_tcp_connect.clicked.connect(self._run_tcp_connect)
        self.btn_http_probe.clicked.connect(self._run_http_probe)
        self.btn_ssl_info.clicked.connect(self._run_ssl_info)
    ...
    def _run_tcp_connect(self):
        target = self.scan_target.text().strip()
        if not target:
            self.scan_output.append("// error: no target specified")
            return
        # determine host and optional port list from custom_flags
        host = target.split(':')[0]
        ports = [22, 80, 443, 8080]
        custom = self.custom_flags.text().strip()
        if custom and '-p' in custom:
            try:
                pstr = custom.split('-p',1)[1].strip().split()[0]
                if ',' in pstr:
                    ports = [int(x) for x in pstr.split(',') if x.isdigit()]
                elif '-' in pstr:
                    a,b = [int(x) for x in pstr.split('-',1)]
        ...
        use_evasion = self.evasion_enabled.isChecked()

        self.scan_output.append(f"// TCP connect scan -> {host} ports={ports} evasion={'ON' if use_evasion else 'OFF'}")
        
        if use_evasion:
            # Import evasion module
            try:
                from core.scanners import tcp_connect_scan_with_evasion, FirewallEvasionConfig
                config = FirewallEvasionConfig()
                config.timing_mode = self.evasion_timing.currentData()
                config.use_decoys = self.evasion_decoys.isChecked()
                config.fragment_packets = self.evasion_fragment.isChecked()
                config.randomize_ports = self.evasion_randomize_ports.isChecked()
                config.randomize_ttl = self.evasion_randomize_ttl.isChecked()
                config.packet_size_variation = self.evasion_packet_size.isChecked()
                
                self.tcp_connect_evasion_worker = TcpConnectEvasionWorker(host, ports, config)
                self.tcp_connect_evasion_worker.result.connect(self._on_tcp_connect_evasion_result)
                self.tcp_connect_evasion_worker.finished.connect(self._on_tcp_connect_finished)
                self.tcp_connect_evasion_worker.start()
            except ImportError:
                self.scan_output.append("// error: firewall evasion module not available")
        else:
            self.tcp_connect_worker = TcpConnectWorker(host, ports)
            self.tcp_connect_worker.result.connect(self._on_tcp_connect_result)
            self.tcp_connect_worker.finished.connect(self._on_tcp_connect_finished)
            self.tcp_connect_worker.start()

    def _on_tcp_connect_result(self, open_ports):
        if open_ports:
            self.scan_output.append(f"Open ports: {open_ports}")
        else:
            self.scan_output.append("No open ports found (quick scan)")

    def _on_tcp_connect_evasion_result(self, result_dict):
        """Handle evasion scan results."""
        if "error" in result_dict:
            self.scan_output.append(f"// error: {result_dict['error']}")
            return
        
        # Display evasion report
        if "evasion_report" in result_dict:
            self.scan_output.append(result_dict["evasion_report"])
        ...
        if open_ports:
            ...
            self.scan_output.append("No open ports found")
        
        # Display stats
        probes = result_dict.get("probes_sent", 0)
        decoys = result_dict.get("decoys_sent", 0)
        self.scan_output.append(f"// Probes sent: {probes}, Decoys sent: {decoys}")

    def _on_tcp_connect_finished(self):
        self.scan_output.append("// TCP connect scan finished.")

    def _run_http_probe(self):
        target = self.scan_target.text().strip()
        if not target:
            self.scan_output.append("// error: no target specified")
            return
        ...
        self.scan_output.append(f"// HTTP probe -> {host}:{port} [running...]")
        self.http_probe_worker = HttpProbeWorker(host, port)
        self.http_probe_worker.result.connect(self._on_http_probe_result)
        self.http_probe_worker.finished.connect(self._on_http_probe_finished)
        self.http_probe_worker.start()

    def _on_http_probe_result(self, info):
        for k, v in info.items():
            self.scan_output.append(f"{k}: {v}")

    def _on_http_probe_finished(self):
        self.scan_output.append("// HTTP probe finished.")

    def _run_ssl_info(self):
        target = self.scan_target.text().strip()
        if not target:
            self.scan_output.append("// error: no target specified")
            return
        ...
        self.scan_output.append(f"// SSL info -> {host}:{port} [running...]")
        self.ssl_info_worker = SslInfoWorker(host, port)
        self.ssl_info_worker.result.connect(self._on_ssl_info_result)
        self.ssl_info_worker.finished.connect(self._on_ssl_info_finished)
        self.ssl_info_worker.start()

    def _on_ssl_info_result(self, info):
        for k, v in info.items():
            self.scan_output.append(f"{k}: {v}")

    def _on_ssl_info_finished(self):
        self.scan_output.append("// SSL info finished.")
